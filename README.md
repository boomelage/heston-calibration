...
4. Run [`calibrator_prototype.py`](src/calibrator_prototype.py) to calibrate once per trading day (over a pooled, moneyness-normalised surface) using the calibration engine [`calibrate_heston.py`](src/calibrate_heston.py). Outputs land under `results/calibrations/<objective>/` (`<objective>` = `price`, the default, or `vol` via `--OBJECTIVE`). Optionally run [`validate_calibrations.py`](src/validate_calibrations.py) to grade the results.
